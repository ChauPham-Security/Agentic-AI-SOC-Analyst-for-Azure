...
def print_model_table(input_tokens, current_model, tier, assumed_output_tokens=500):
    ...
    for name, info in guardrails.ALLOWED_MODELS.items():
        ...
        print(
            ...
            f"out_max: {info['max_output_tokens']:<6} | est_cost: {money(est)}{tag}"
        )
    print("")
...
def choose_model(model_name, input_tokens, tier=CURRENT_TIER, assumed_output_tokens=500, interactive=True):
    ...
    while True:
        prompt = (
            ...
            f"(Press Enter to continue / Type the exact model name you choose / 'list' / 'exit'):{Fore.WHITE} "
        )
        choice = input(prompt).strip()

        if choice.lower() in {"exit", "quit", "q"}:
            print(f"{Fore.YELLOW}Exiting model selection.{Fore.WHITE}")
            return None
        ...
        print("Press Enter to continue, Type the exact model name you choose, or type 'list' or 'exit'.")
